...
* A node configuration file entitled `node.conf`, configured as per [Node configuration](corda-configuration-file)
...
The remaining files and folders described in [Node folder structure](node-structure) will be generated at runtime.
...
* `name` <string>
    ...
    * The legal identity name of the Corda node. (see [myLegalName](corda-configuration-file#corda-configuration-file-mylegalname))
...
* `p2pAddress` <string> <**required if p2pPort not specified**>
    ...
    * The address/port the node uses for inbound communication from other nodes. (see [p2pAddress](corda-configuration-file#corda-configuration-file-p2paddress))
...
* `p2pPort` <integer>
    ...
    * The port the node uses for inbound communication from other nodes.  Assumes the address is `localhost`. (see [p2pAddress](corda-configuration-file#corda-configuration-file-p2paddress))
...
* `rpcSettings` <config>
    ...
    * Specifies RPC settings for the node. (see [rpcSettings](corda-configuration-file#corda-configuration-file-rpc-settings))
...
* `notary` <config> (see [notary](corda-configuration-file#corda-configuration-file-notary))
...
* `devMode` <boolean>
    ...
    * When true enables development mode. (see [devMode](corda-configuration-file#corda-configuration-file-dev-mode))
...
* `rpcUsers` <list>
    ...
    * Set the RPC users for this node. (see [rpcUsers](corda-configuration-file#corda-configuration-file-rpc-users))
...
* `sshdPort` <integer>
    ...
    * Specifies the port for sshd communication. (see [sshd](corda-configuration-file#corda-configuration-file-sshd))
...
To extend node configuration beyond the properties defined in the `deployNodes` task use the `configFile` property with the path (relative or absolute) set to an additional configuration file.
                    This file should follow the standard [Node configuration](corda-configuration-file) format, as per node.conf. The properties from this file will be appended to the generated node configuration. Note, if you add a property already created by the ‘deployNodes’ task, both properties will be present in the file.
...
To specify design/data-model-upgrades/package-namespace-ownership configuration, the optional `networkParameterOverrides` and `packageOwnership` blocks can be used, similar to the configuration file used in [Network Bootstrapper](network-bootstrapper):
...
The Cordform `signing` entry can be used to override and customise the signing of CorDapp JARs.
                    Signing the CorDapp enables its contract classes to use signature constraints instead of other types of the constraints [API: Contract Constraints](api-contract-constraints).
...
* `options` any relevant parameters of [SignJar ANT task](https://ant.apache.org/manual/Tasks/signjar) and [GenKey ANT task](https://ant.apache.org/manual/Tasks/genkey.html),
...
* `generateKeystore` the flag to generate a keystore, it is set to `false` by default. If set to `true` then ad hock keystore is created and its key isused
                            ...
                            `dname` and `keyalg` are required. Other options are described in [GenKey task](https://ant.apache.org/manual/Tasks/genkey).
...
Contracts classes from signed CorDapp JARs will be checked by signature constraints by default.
                    ...
                    the list will generate *include_whitelist.txt* file used internally by [Network Bootstrapper](network-bootstrapper) tool.
                    Refer to [API: Contract Constraints](api-contract-constraints) to understand implication of different constraint types before adding `includeWhitelist` to `deployNodes` task.
...
The nodes’ webservers will not be started. Instead, you should interact with each node via its shell over SSH
                (see the [node configuration options](corda-configuration-file)). You have to enable the shell by adding the
